...
@SpringBootTest(properties = {
        "cuscrud.database.url=jdbc:h2:mem:update_inventory_user_endpoint;MODE=PostgreSQL;DB_CLOSE_DELAY=-1",
        "cuscrud.database.user=sa",
        "cuscrud.database.password=test",
        "cuscrud.auth.jwt-secret=test-jwt-secret-for-update-inventory-user"
})
@AutoConfigureMockMvc
class InventoriesUsersUpdateControllerTest {
    ...
    /**
     * Prepara o schema minimo antes de cada teste do endpoint de atualizacao de role de usuarios do inventario.
     * Entrada: nenhuma.
    ...
    /**
     * Verifica que PATCH /api/v1/inventories/{inv_id}/users/{user_id} atualiza a role de um usuario vinculado.
     * Entrada: owner autenticado e usuario alvo com role `1`.
     * Esperado: status 200 com inventario e usuario retornados no corpo.
     *
     * @throws Exception quando a execucao do request de teste falha.
     */
    @Test
    void shouldUpdateRoleOfInventoryUserWhenAuthenticatedUserIsOwner() throws Exception {
        ...
        mockMvc.perform(patch("/inventories/{inv_id}/users/{user_id}", inventoryId, targetUserId)
    ...
    /**
     * Verifica que PATCH /api/v1/inventories/{inv_id}/users/{user_id} exige autenticacao.
     ...
     */
    @Test
    void shouldRequireAuthenticationToUpdateInventoryUser() throws Exception {
        mockMvc.perform(patch("/inventories/{inv_id}/users/{user_id}", UUID.randomUUID(), UUID.randomUUID())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
                                  "role": 2
    ...
    /**
     * Verifica que PATCH /api/v1/inventories/{inv_id}/users/{user_id} rejeita solicitante sem role owner.
    ...
    /**
     * Verifica que PATCH /api/v1/inventories/{inv_id}/users/{user_id} rejeita usuario alvo inexistente.
    ...
    /**
     * Verifica que PATCH /api/v1/inventories/{inv_id}/users/{user_id} rejeita usuario sem vinculo no inventario.
    ...
    /**
     * Verifica que PATCH /api/v1/inventories/{inv_id}/users/{user_id} rejeita alteracao da propria role do owner.
     ...
     */
    @Test
    void shouldReturnConflictWhenOwnerAttemptsToChangeOwnRole() throws Exception {
        UUID ownerId = insertUser("Joao Silva", "joao@example.com", "senhaforte456");
        UUID inventoryId = UUID.fromString("123e4567-e89b-12d3-a456-426614174000");
        insertInventory(inventoryId, "Estoque da Loja");
        insertInventoryAccess(ownerId, inventoryId, 0);
        ...
        mockMvc.perform(patch("/inventories/{inv_id}/users/{user_id}", inventoryId, ownerId)
                        .header("Authorization", "Bearer " + issuedJwtToken.token())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
    ...
    /**
     * Verifica que PATCH /api/v1/inventories/{inv_id}/users/{user_id} rejeita role fora do conjunto permitido.
